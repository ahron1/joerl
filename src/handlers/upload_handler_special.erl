%% upload new images from client to server/db using a system account as the user
%todo - make a separate dir for hosting, which contains soft links to images, in a flat file structure (without daily dirs)

-module(upload_handler_special).
-export([init/2]).

init(Req0, Opts) ->
	%skip the next step and just directly hard-assign the values.
	%{CookieStatus, User} = entry_helpers:check_session_cookie(Req0),
	%don't check if user is logged in before processing uploaded file/info
	CookieStatus = has_session_cookie,
	[{User}]= db_helpers:get_sys_user(),
...
file_storage(FileSize, MinFileSize,  MaxFileSize, FinalFileName, FileNameWithPath, TempFile,  FileData, User, ImageUri, Adj1, Adj2) when (FileSize < MaxFileSize) andalso  (FileSize > MinFileSize) ->
	...
	{_NewImageId, _Adj1Text, _Adj2Text} = db_helpers:image_details_to_db_special(User, FinalFileName, ImageUri, FileNameWithPath, Adj1, Adj2),

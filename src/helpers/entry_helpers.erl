...
-export([check_session_cookie/1, check_has_body/1, extract_login_pw/1, extract_login_pw_fp/1, extract_signup_details/1, extract_spot_signup_details/1 ]).
...
%%extract login/pw/fingerprint from req and return as tuple
extract_login_pw_fp(OriginalRequest) ->
	{ok, _Headers1, Req2} = cowboy_req:read_part(OriginalRequest),
	{ok, FormLogin, Req3} = cowboy_req:read_part_body(Req2),
	...
	{ok, FingerPrint, _Req7} = cowboy_req:read_part_body(Req6),
	{FormLogin, FormPassword, FingerPrint}.
...
%% extract user details from spot signup form - difference with the regular signup is the pw field
extract_spot_signup_details(OriginalRequest) ->
	{ok, _Headers1, Req2} = cowboy_req:read_part(OriginalRequest),
	{ok, FormLogin, Req3} = cowboy_req:read_part_body(Req2),
	...
	{ok, _Headers3, Req6} = cowboy_req:read_part(Req5),
	{ok, FirstName, _Req7} = cowboy_req:read_part_body(Req6),

%	{ok, _Headers4, Req8} = cowboy_req:read_part(Req7),
%	{ok, InviterEmail, _Req9} = cowboy_req:read_part_body(Req8),
%	{ok, _Headers4, Req8} = cowboy_req:read_part(Req7),
%	{ok, BirthYear, _Req9} = cowboy_req:read_part_body(Req8),

	SignupDetails = {FormLogin, FormPassword, FirstName},
	erlang:display(SignupDetails),
	SignupDetails.
